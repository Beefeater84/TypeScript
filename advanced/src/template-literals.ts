...
- В вашей шпаргалке есть пример example_string:Example: ${string}. Объясните, почему example_string = 'Without Example prefix';` вызывает ошибку компиляции.
...
- В функции applyStyle используется тип {Size}-{Color}. Предположим, мы хотим также поддерживать стили с обратным порядком, например, primary-small. Как бы вы изменили определение типа для applyStyle, чтобы это было возможно?
